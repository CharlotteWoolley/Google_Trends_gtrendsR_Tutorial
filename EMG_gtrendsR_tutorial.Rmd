---
title: "gtrendsR tutorial for beginners - EMG journal club"
...
using the gtrendsR package and to demonstrate its potential application within the field of musculoskeletal
biology. A paper entitiled "Increasing Public Interest in Stem Cell Injections for Osteoarthritis of the Hip and Knee: A Google Trends Analysis" by Strotman *et al*., 2019 is examined.
...
### Example 1 - Investigate UK searches for 'arthritis' over the past week

Use the gtrends function with the appropriate arguements. 
```{r}
arthritis <- gtrends(keyword = 'arthritis', geo = 'GB', time = 'now 7-d')
```
...
```{r}
arthritis_iot <- as.data.frame(arthritis$interest_over_time)

arthritis_iot <- arthritis_iot %>%
mutate(date = as.Date(date),
        day = factor(lubridate::wday(ymd(date), label = TRUE),
        levels = c('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun')))

ggplot(arthritis_iot, aes(x = day, y = hits, group = day)) +
        geom_boxplot() +
        stat_summary(fun.y=mean, colour="red", geom="point", shape=18, size=3.5) +
        labs(title = "Google searches in Great Britain for 'arthritis' over the past week", 
             x = "Day", 
             y = "Search ratio") +
theme_bw()
```
...
### Example 2 - Investigate UK searches for 'arthritis' over the past 5 years

Use the gtrends function with the appropriate arguements. 
```{r}
arthritis2 <- gtrends(keyword = 'arthritis', geo = 'GB', time = 'today+5-y')
```

Now we can extract the 'interest over time' database and plot it using ggplot
```{r}
arthritis2_iot <- as.data.frame(arthritis2$interest_over_time)
arthritis2_iot$date <- as.Date(arthritis2_iot$date)
ggplot(arthritis2_iot, aes(x=date, y = hits)) +
        geom_line() +
        labs(title = "Google searches in Great Britain for 'arthritis' in the last 5 years", 
             x = "Year", 
             y = "Search index") +
...
There might be a seasonal pattern to searching for 'arthritis'. To view this
pattern more clearly, we can group the data by month and summarise it as a boxplot. 
```{r}
arthritis2_iot <- arthritis2_iot %>%
        mutate(month = lubridate::month(ymd(date), label = TRUE, abbr = FALSE))
ggplot(arthritis2_iot, aes(x = month, y = hits, group = month)) +
        geom_boxplot() +
        stat_summary(fun.y=mean, colour="red", geom="point", shape=18, size=3.5) +
        labs(title = "Google searches in Great Britain for 'arthritis' by month", 
             x = "Month", 
             y = "Hits") +
        theme_bw() +
        theme(axis.text.x=element_text(angle=90,hjust=1))
...
'arthritis' the most. To do this, the 'interest by region' dataframe needs to be extracted.
```{r}
arthritis2_ibr <- as.data.frame(arthritis2$interest_by_region)
```
...
```{r}
arthritis2_ibr$location <- factor(arthritis2_ibr$location, 
levels = arthritis2_ibr$location[order(arthritis2_ibr$hits, decreasing = TRUE)])

ggplot(arthritis2_ibr, aes(x=location, y = hits)) +
        geom_bar(stat='identity', fill = c('yellow','green', 'blue', 'red')) +
        labs(title = "Google searches in Great Britain for 'arthritis' over the last 5 years", 
             x = "Region", 
             y = "Search index") +
        scale_y_continuous(breaks=c(seq(from=0, to=100, by=10))) +
theme_bw()
...
To narrow this down even further, we can look at the top 20 cities that searched the most for 'arthritis'.
```{r}
arthritis2_ibc <- as.data.frame(arthritis2$interest_by_city)

arthritis2_ibc$location <- factor(arthritis2_ibc$location, 
                                levels = arthritis2_ibc$location[order(arthritis2_ibc$hits, decreasing = TRUE)])
ggplot((arthritis2_ibc %>% drop_na()), aes(x=location, y = hits)) +
        geom_bar(stat='identity') +
        labs(title = "Google searches in the UK for 'arthritis' over the last 5 years", 
             x = "City", 
             y = "Search index") +
        scale_y_continuous(breaks=c(seq(from=0, to=100, by=10))) +
        theme_bw() +
...
We can also have a look at the related queries to 'arthritis'. In this
...
```{r}
arthritis2_rq <- as.data.frame(arthritis2$related_queries)

top_queries <- subset(arthritis2_rq, arthritis2_rq$related_queries == 'top')
...
ggplot(top_queries, aes(x=value, y = subject)) +
        geom_bar(stat='identity') +
        labs(title = "Top related Google queries to 'arthritis' in the UK over the last 5 years", 
             x = "Related queries", 
             y = "Search index") +
        scale_y_continuous(breaks=c(seq(from=0, to=100, by=10))) +
        theme_bw() +
...
### Example 3 - Investigate worldwide searches for 'arthritis' since 2010

Use the gtrends function with the appropriate arguements. 
```{r}
arthritis3 <- gtrends(keyword = 'arthritis', time = "2010-01-01 2019-01-01")
```

Now we can extract the 'interest over time' database and plot it using ggplot
```{r}
arthritis3_iot <- as.data.frame(arthritis3$interest_over_time)
arthritis3_iot$date <- as.Date(arthritis3_iot$date)
arthritis3_iot$time <- 1:nrow(arthritis3_iot)
ggplot(arthritis3_iot, aes(x=date, y = hits)) +
        geom_line() +
        labs(title = "Google searches worldwide for 'arthritis' in the last 5 years", 
             x = "Year", 
             y = "Search index") +
        scale_y_continuous(breaks=c(seq(from=0, to=100, by=10))) +
        scale_x_date(date_breaks="1 year", date_labels = "%Y") +
...
There is an obvious increase in interest in 'arthritis'! Now to see which countries are most interested...
```{r}
arthritis3_ibr <- as.data.frame(arthritis3$interest_by_country)
arthritis3_ibr$location <- factor(arthritis3_ibr$location, 
                                levels = arthritis3_ibr$location[order(arthritis3_ibr$hits, decreasing = TRUE)])
ggplot(arthritis3_ibr %>% drop_na(), aes(x=location, y = hits)) +
        geom_bar(stat='identity') +
        labs(title = "Google searches for 'arthritis' worldwide", 
             x = "Region", 
             y = "Search index") +
        scale_y_continuous(breaks=c(seq(from=0, to=100, by=10))) +
        theme_bw() +
...
```{r}
arthritis3_ibr$region <- ifelse(arthritis3_ibr$location == 'United States','USA', 
                                ifelse(arthritis3_ibr$location == 'United Kingdom', 'UK', 
                                        ifelse(arthritis3_ibr$location == 'Czechia', 'Czech Republic', 
                                                as.character(arthritis3_ibr$location))))
```
...
```{r, message=FALSE}
new_map <- full_join(map_data("world"), arthritis3_ibr)
...
## **Journal club paper - Strotman *et al*., 2019**

Aim: Quantify worldwide public interest in stem cell injections for osteoarthritis since 2010

Results: Since 2010 there has been statistically significant rise in Google searches related to stem cells and osteoarthritis of the hip and knee. 

Limitations: 

* There has been a statistically significant rise in google search query volume in pretty much everything since 2010!
* Does not control for caching effects
* Nowhere near enough detail in methods (category, method of data extraction, date of cache etc)

Advances:

* Allows a trend in public interest to be inferred

```{r}
arthritis_inj <- gtrends(keyword = 'arthritis stem cell injection', time = "2010-01-01 2019-01-01")
```

Now we can extract the 'interest over time' database and plot it using ggplot
```{r}
arthritis_inj_iot <- as.data.frame(arthritis_inj$interest_over_time)
arthritis_inj_iot$date <- as.Date(arthritis_inj_iot$date)
arthritis_inj_iot$time <- 1:nrow(arthritis_inj_iot)
ggplot(arthritis_inj_iot, aes(x=date, y = hits)) +
        geom_line() +
        labs(title = "Google searches worldwide for 'arthritis stem cell injection' since 2010", 
             x = "Year", 
             y = "Search index") +
        scale_y_continuous(breaks=c(seq(from=0, to=100, by=10))) +
        scale_x_date(date_breaks="1 year", date_labels = "%Y") +
...
Try linear regression to see if the increase in 'arthritis stem cell injection' hits over time is statistically significant
```{r}
mod1 <- lm(hits ~ time,
           data = arthritis_inj_iot)
summary(mod1)
```

Try linear regression to see if the increase in 'arthritis' hits over time is statistically significant
```{r}
mod2 <- lm(hits ~ time,
           data = arthritis3_iot)
summary(mod2)
...
Finally, extract gtrends data and try linear regression to see if the increase in 'arthritis xray' hits over time is statistically significant

```{r}
arthritis_x <- gtrends(keyword = 'arthritis xray', time = "2010-01-01 2019-01-01")
arthritis_x_iot <- as.data.frame(arthritis_x$interest_over_time)
arthritis_x_iot$date <- as.Date(arthritis_x_iot$date)
arthritis_x_iot$time <- 1:nrow(arthritis_x_iot)

mod3 <- lm(hits ~ time,
           data = arthritis_x_iot)
summary(mod3)
```
...
* Strotman, P.K., Novicoff, W. M., Nelson, S. J. & Browne, J A. (2019). Increasing Public Interest in Stem Cell Injections for Osteoarthritis of the Hip and Knee: A Google Trends Analysis. The Journal of Arthroplasty. 34. 10.1016/j.arth.2019.03.002. 
